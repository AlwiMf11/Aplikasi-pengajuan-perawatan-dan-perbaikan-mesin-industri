@extends('layouts.app', ['activePage' => 'profile', 'titlePage' => __('Edit Perawatan')])
...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="/permintaan/perawatan/ubah" autocomplete="off" class="form-horizontal">
            @csrf
            <input type="hidden" name="id" value="{{$data->id}}">

            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">Edit Perawatan</h4>
                <p class="card-category">{{ __('Perawatan information') }}</p>
              </div>
              <div class="card-body ">
                @if (session('status'))
                  <div class="row">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('judul') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('judul') ? ' is-invalid' : '' }}" value="{{$data->judul}}" name="judul" id="input-judul" type="text" placeholder="{{ __('judul') }}"  required="true" aria-required="true"/>
                      @if ($errors->has('judul'))
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('keterangan') ? ' has-danger' : '' }}">
                      <textarea class="form-control{{ $errors->has('keterangan') ? ' is-invalid' : '' }}" name="keterangan" id="input-keterangan" type="text" placeholder="Keterangan"  required="true" aria-required="true" rows="4"> {{$data->keterangan}} </textarea>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('tgl_permintaan') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('tgl_permintaan') ? ' is-invalid' : '' }}" value="{{$data->tgl_permintaan}}" name="tgl_permintaan" id="input-tgl_permintaan" type="date" placeholder="{{ __('tgl_permintaan') }}"  required="true" aria-required="true"/>
